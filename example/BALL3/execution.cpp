...
void execute(Vehicle &vehicle_list,Module *module_list,double sim_time,
			 ...
			 int num_ball,ofstream &ftraj,char *title,bool traj_merge);
...
int main() 
{
	...
	int num_ball; //number of ball objects
	...
	Document *doc_ball6=NULL;  //array for documenting BALL module-variables of 'input.asc'
	bool document_ball6=false; //true if array doc_ball6 was created
	...
	bool document_ball=true; //write  to 'doc.asc' ball module-variables only once
	...
	number_objects(input,num_vehicles,num_ball);
	...
	{
		//Loading pointers of the vehicle-object classes ('Ball') into 
		...
		if(!strcmp(vehicle_name,"BALL")&&one_screen_banner)
		{
		...
		//currently only the BALL plot files contain any data
		if(strstr(options,"y_plot"))
		{
			if(!strcmp(vehicle_name,"BALL"))
			{
				char index[CHARN]; //
		...
		//composing documentation of 'ball' but do it only once for BALL
		if(strstr(options,"y_doc"))
		{
			if(!strcmp(vehicle_name,"BALL")&&document_ball)
			{
				document_ball=false;
				int size=NBALL;
				doc_ball6=new Document[size];
				if(doc_ball6==0){cerr<<"*** Error: doc_ball6[] allocation failed *** \n";system("pause");exit(1);}
				vehicle_list[i]->document(fdoc,title,doc_ball6);
				document_ball6=true;
			}
		}
		...
		combus[i]=vehicle_list[i]->loading_packet_init(num_ball);
	...
	if(strstr(options,"y_traj")&&one_traj_banner)
	{
		...
		traj_banner(ftraj,combus,title,num_vehicles,num_ball);
		...
		if(strstr(options,"y_doc")){
			...
			document_input(doc_ball6);
			if(document_ball6)delete [] doc_ball6;
		}
	///////////////////////////////////////////////////////////////////////////////	
	...
	execute(vehicle_list,module_list,sim_time,
			 ...
			 plot_ostream_list,combus,status,num_ball,ftraj,title,
			 traj_merge);
	///////////////////////////////////////////////////////////////////////////////	
	/////////////////////// Final Calculations ////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////////	
		for(int i=0;i<num_vehicles;i++)
		{
			...
			if(health){
				for (int j=0;j<num_modules;j++){						
						// No termination modules needed for BALL3
				}
			}
		}
	...
	{
		merge_plot_files(plot_file_list,num_ball,title);
	}
	//Deallocate dynamic memory
...
//								and their respective type - ball
...
//				*plot_ostream_list = output file-steam list of 'ploti.asc' for each individual ball 
//								ball object
//				*combus = commumication bus
//				*status = health of vehicles
//				num_ball = number of 'Ball' objects
...

			{
				...
				for(int j=0;j<num_modules;j++)
				{
					if(module_list[j].name=="environment")
						...
						vehicle_list[i]->forces(int_step);
				...
				combus[i]=vehicle_list[i]->loading_packet(num_ball);
