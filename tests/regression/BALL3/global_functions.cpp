...
//
//Contains the global functions for the Ball6 simulation.
//
...
//					&num_ball, number of ball balls					
//
//010330 Created by Peter H Zipfel
//020920 Added check for illigal '=' signs and missing numerical entries, PZi
///////////////////////////////////////////////////////////////////////////////

void number_objects(fstream &input,int &num_vehicles,int &num_ball)
{
	char read[CHARN];
	char line_clear[CHARL];
	num_vehicles=0;
	...
	do{
		input>>read;
		input.getline(line_clear,CHARL,'\n');
		if (!strcmp(read,"BALL")) num_ball++;

	}while((num_ball)<num_vehicles);
...
Cadac *set_obj_type(fstream &input,Module *module_list,int num_modules)				   
{
	...
	if (!strcmp(temp,"BALL"))
	{
		//the pointer 'obj' is allocated the type 'Ball' 
		obj=new Ball(module_list,num_modules);
...
//011022 Changed treatment of plot_file_list;'num_vehicles'replaced by'num_ball'PZi
//020304 Correction in first while loop, PZi
///////////////////////////////////////////////////////////////////////////////
void merge_plot_files(string *plot_file_list,int num_ball,char *title)
{	
	char line_clear[CHARL];
	char line[CHARL];
	char buff[CHARN];
	...
	file_istream_list=new ifstream[num_ball];
...
//				if(file_istream_list[i].gcount()) fmerge<<line;
	...
	fmerge.close();
	for(i=0;i<num_ball;i++) file_istream_list[i].close();
...
//to be appended, limiting ball vehicles to 9 each (e.g.: SBEG1_t1)
//Accomodates 3x1 vectors
//
//010207 Created by Peter Zipfel
//011129 Adapted to Ball6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
				 ...
				 int nball)
{
	...
	string id;
	int loc;
	int i_ball=0; //first ball packet index in vehicle_list
	int ncom_ball6; //number of module variables in 'com_ball6'
	Variable *data_ball; //Variable array stored in 'Packet data' of type Ball
	int i(0),j(0);

	ftraj<<"1"<<title<<" "<< __DATE__ <<" "<< __TIME__ <<"\n";

	//find first ball packet index in 'combus'
	...
	//number of data elements in 'com_ball'
	ncom_ball6=combus[i_ball].get_ndata();

	//picking the first data packet for label extraction
	data_ball=combus[i_ball].get_data();

	//determining the total number of vector variables in com_ball6
	for(j=0;j<ncom_ball6;j++)
	{
		buff1=data_ball[j].get_name();
		if(isupper(buff1[0])) p++;
	}
	//number of variables (with vectors componentalized and only one 'time' label))
	int nvariables=ncom_ball6*nball+2*p*nball-(nball-1);
	...
	for(i=0;i<num_vehicles;i++)
	{
		//determining 'Ball' object
		id=combus[i].get_id();
		loc=id.find("m");
		if(!loc)
		//'Ball' object
		{
			for(int k=0;k<ncom_ball6;k++)
			{
				buff1=data_ball[k].get_name();
...
void traj_data(ofstream &ftraj,Packet *combus,int num_vehicles,bool merge)
{
	...
	{
		//determining wether 'Ball' object
		id=combus[i].get_id();
		loc=id.find("m");
		if(!loc)
		//'Ball' object
...

{
	...
	int p=0;
	string id;
	int nball=0; //number of ball packets 
	int loc;
	int i_ball=0; //first ball packet index in vehicle_list
	int ncom_ball6; //number of module variables in 'com_ball6'
	Variable *data_ball; //Variable array stored in 'Packet data' of type Ball
	...
	//determining the number of ball packets
	for(i=0;i<num_vehicles;i++)
	{
		id=combus[i].get_id();
		loc=id.find("m");
	...
	//'Ball' object labels (exclude 'time')
	for(k=1;k<ncom_ball6;k++)
	{
		buff1=data_ball[k].get_name();
...
///////////////////////////////////////////////////////////////////////////////
void document_input(Document *doc_ball6)
{
	...
	input.setf(ios::left);
	do{
		fcopy>>buffo;;//buffo gets BALL, ENDTIME STOP and //comments

		//finding BALL object
		if(!strcmp(buffo,"BALL")){
			input<<'\t'<<buffo;		//writes BALL line
			fcopy.getline(line_clear,CHARL,'\n');
			input<<line_clear<<'\n';

			//inside BALL loop until 'END' is reached
			do{
				fcopy>>buffn;
				int dum=1;
				...
				//inserting 'END' with only one tab
					...
					for(int k=0;k<(NBALL);k++){
						if(!strcmp(doc_ball6[k].get_name(),"end_array"))break;
						if(!strcmp(doc_ball6[k].get_name(),buffn)){
							fcopy.getline(line_clear,CHARL,'\n'); 							
							input<<"    //";
							if(!strcmp(doc_ball6[k].get_type(),"int"))input<<"'int' ";
							input<<doc_ball6[k].get_def();
							input<<"  module ";
							input<<doc_ball6[k].get_mod();
							input<<'\n';
							*buff_stoch=NULL;
							def_found=true;
						}
